...
class NewtonEnv(Environment):
    ...
    def __init__(self, cfg_env: NewtonEnvCfg, device="cuda:0"):
        ...
        # reusable host buffer for the full-qd position-target vector
    ...
    def step(self, pd_target, hand_pose=None):
        ...
        if hand_pose is not None:
            logger.info(f"Hand pose--> {hand_pose}")

        if self.use_implicit_pd:
            self._target_pos_host[:] = 0.0
    ...
    def save_frame(self, path: str, width: int = 640, height: int = 480) -> bool:
        """Newton deploy runs headless with no viewer; frame capture is not wired
        (frames are evidence, never load-bearing). Returns False so callers proceed."""
        logger.debug("[NewtonEnv] save_frame not supported on the headless Newton backend")
        return False
